...
If you're using Nuxt (or Nitro directly), configure the origin via runtime config and let the `workflow/nitro` module map it to `WORKFLOW_LOCAL_BASE_URL` internally at runtime.
