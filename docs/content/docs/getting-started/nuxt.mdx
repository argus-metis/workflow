...
## Custom domains / ports (Nuxt runtime config)

If you're running Nuxt behind a reverse proxy, tunnel, custom hostname, or non-standard port, configure the local world queue origin using Nuxt `runtimeConfig`.
...
export default defineNuxtConfig({
  modules: ["workflow/nuxt"],
  runtimeConfig: {
    // Server-only (recommended): this is internal queue traffic, not browser config.
    workflow: {
