...
export class Encoder extends Decoder {
  constructor(options) {
    ...
    const encodeUtf8 = ByteArray.prototype.utf8Write
      ? (string, position) =>
          target.utf8Write(string, position, target.byteLength - position)
      : textEncoder && textEncoder.encodeInto
        ? (string, position) =>
            textEncoder.encodeInto(string, target.subarray(position)).written
        : false;
    ...
    this.findCommonStringsToPack = () => {
      ...
      return (options) => {
        const threshold = (options && options.threshold) || 4;
    ...
    const encode = (value) => {
      ...
      if (type === 'string') {
        ...
        if (!this.alwaysUseFloat && value >>> 0 === value) {
          ...
          // negative integer, 31-bit or less
          if (value >= -0x18) {
        ...
        if (!value) target[position++] = 0xf6;
          ...
          // Check instance extensions first (supports predicate functions for plain objects)
          if (encoder.instanceExtensions) {
            for (let i = 0, l = encoder.instanceExtensions.length; i < l; i++) {
              ...
              // Try instanceof for constructors with valid prototype, or call as predicate function
              let matches = false;
              // Only use instanceof if extensionClass has a valid prototype (is a constructor)
              if (
                typeof extensionClass === 'function' &&
                extensionClass.prototype !== undefined
              ) {
                try {
                  matches = value instanceof extensionClass;
                } catch (e) {
                  // instanceof failed (e.g., bound function or arrow function), try as predicate
                  matches = false;
                }
              }
              // If instanceof didn't match, try calling as predicate function
              if (!matches && typeof extensionClass === 'function') {
                try {
          ...
          if (constructor === Object || constructor === encoder.global.Object) {
            ...
            constructor === Array ||
            constructor === encoder.global.Array
          ) {
            length = value.length;
            if (length < 0x18) {
            ...
            constructor === Map ||
            constructor === encoder.global.Map
          ) {
            if (
              this.mapsAsObjects
    ...
    this.encodeAsIterable = (value, options) =>
      startEncoding(value, options, encodeObjectAsIterable);
    this.encodeAsAsyncIterable = (value, options) =>
      startEncoding(value, options, encodeObjectAsAsyncIterable);
    ...
    function startEncoding(value, options, encodeIterable) {
      if (options && options.chunkThreshold)
        ...
        chunkThreshold = continuedChunkThreshold = options.chunkThreshold; // we start with a smaller threshold to get initial bytes sent quickly
    ...
    async function* encodeObjectAsAsyncIterable(value, iterateProperties) {
      for (const encodedValue of encodeObjectAsIterable(
        value,
        iterateProperties,
        true
      )) {
  ...
  updateSharedData() {
    ...
    const saveResults = this.saveShared(
      sharedData,
      (existingShared) =>
