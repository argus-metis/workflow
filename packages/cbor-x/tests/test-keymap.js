...
for (let i = 0; i < 1000; i++)
  large.push({ t: 100 + i, n: '1', vs: 'value-' + i });
...
function perfTest(data, label) {
  ...
  compare(
    `PreMap No Recs: ${label}`,
    test(data, { useRecords: false }, true),
    basic
  );
  compare(
    `KeyMap No Recs: ${label}`,
    test(data, { useRecords: false, keyMap: senmlKeys }),
    basic
  );
  compare(
    `Optima No Recs: ${label}`,
    test(data, { useRecords: false, _keyMap: senmlKeys }),
    basic
  );
  compare(`Basic  Wi Recs: ${label}`, test(data, { useRecords: true }), basic);
  compare(
    `PreMap Wi Recs: ${label}`,
    test(data, { useRecords: true }, true),
    basic
  );
  compare(
    `KeyMap Wi Recs: ${label}`,
    test(data, { useRecords: true, keyMap: senmlKeys }),
    basic
  );
  compare(
    `Optima Wi Recs: ${label}`,
    test(data, { useRecords: true, _keyMap: senmlKeys }),
    basic
  );
}
...
function test(data, opts, preMap, its = 10) {
  ...
  let decode = (b) =>
    preMap ? preMapDecode(cbor.decode(b)) : cbor.mapDecode(b);
  let encode = (d) =>
    preMap ? cbor.encode(preMapEncode(d)) : cbor.mapEncode(d);
