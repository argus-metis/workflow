  ...
  for (const [label, testData] of Object.entries(tests)) {
    test(label, () => {
      ...
      assert.deepStrictEqual(
        encoder.decode(encoded, encoded.length, true),
        testData,
        'full buffer decodes well'
      );
      ...
      try {
        ...
        if (err.incomplete !== true) {
          assert.fail(
            `Should throw an error with .incomplete set to true, instead threw error <${err}>`
          );
        } else {
      ...
      assert.fail(
        `Should throw an error with .incomplete set to true, instead returned value ${JSON.stringify(value)}`
      );
    });
  }
});
