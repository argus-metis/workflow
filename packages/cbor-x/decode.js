...
export class Decoder {
  constructor(options) {
    if (options) {
      ...
      // Process instance-level extensions
      if (options.extensions) {
        this.instanceExtensions = options.extensions;
        // Build a map of tag -> decode function for quick lookup
        this.instanceTags = {};
        for (const ext of options.extensions) {
          if (ext.tag !== undefined && ext.decode) {
            this.instanceTags[ext.tag] = ext.decode;
          }
        }
      }
    }
    // Store the global scope for type construction (defaults to globalThis)
    this.global = (options && options.global) || globalThis;
  ...
  encodeKey(key) {
    return this.keyMap && Object.hasOwn(this.keyMap, key)
      ? this.keyMap[key]
      : key;
  }
  ...
  encodeKeys(rec) {
    ...
    for (const [k, v] of Object.entries(rec))
      map.set(Object.hasOwn(this._keyMap, k) ? this._keyMap[k] : k, v);
    return map;
  }
...
export function read() {
        ...
        switch (majorType) {
          ...
          case 5: {
            ...
            if (currentDecoder.mapsAsObjects) {
              ...
              const map = new currentDecoder.global.Map();
              if (currentDecoder.keyMap) {
                let i = 0;
                while ((key = read()) != STOP_CODE) {
                  if (i++ >= maxMapSize) {
  ...
  switch (majorType) {
    ...
    case 6: {
      ...
      const extension =
        (currentDecoder.instanceTags && currentDecoder.instanceTags[token]) ||
        currentExtensions[token];
      if (extension) {
...
currentExtensions[0] = (dateString) => {
  ...
  return new currentDecoder.global.Date(dateString);
};

currentExtensions[1] = (epochSec) => {
  // numeric date extension
  return new currentDecoder.global.Date(Math.round(epochSec * 1000));
};
...
currentExtensions[27] = (data) => {
  ...
  // Use currentDecoder.global.Error as fallback to support VM contexts
  return (
    currentDecoder.global[data[0]] ||
    currentDecoder.global.Error ||
    Error
  )(data[1], data[2]);
};
...
currentExtensions[258] = (array) => new currentDecoder.global.Set(array); // https://github.com/input-output-hk/cbor-sets-spec/blob/master/CBOR_SETS.md
...
function registerTypedArray(TypedArray, tag) {
  ...
  const dvMethod = 'get' + typedArrayName.slice(0, -5);
  let bytesPerElement;
  ...
  for (let littleEndian = 0; littleEndian < 2; littleEndian++) {
    ...
    currentExtensions[littleEndian ? tag : tag - 4] =
      bytesPerElement == 1 || littleEndian == isLittleEndianMachine
        ? (buffer) => {
            const ResolvedTypedArray =
              currentDecoder.global[typedArrayName] || TypedArray;
            if (!ResolvedTypedArray)
        ...
        : (buffer) => {
            ...
            const ta = new ResolvedTypedArray(elements);
