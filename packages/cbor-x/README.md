...
This is a fork of [cbor-x](https://github.com/kriszyp/cbor-x) with modifications for the Vercel Workflow runtime.

**Tracking fork:** https://github.com/vercel-labs/cbor-x

## Fork Changes

This fork adds the following features to support VM context isolation and custom type handling:

### 1. `global` option

The `global` option allows specifying a custom global scope for type construction during decoding. This is essential for VM contexts where `instanceof` checks fail across context boundaries.

```javascript
import { Decoder } from '@workflow/cbor-x';
import vm from 'node:vm';

const context = vm.createContext({ Date, Map, Set, Error, RegExp });
...
// Decoded Date objects will be instances of context.Date, not globalThis.Date
const result = decoder.decode(encoded);
```

### 2. Per-instance `extensions` option

Extensions can now be passed per-instance to the Encoder/Decoder constructor, and support predicate functions (not just constructor `instanceof` checks). This allows custom type handling that works across VM context boundaries.

```javascript
import { Encoder, Decoder } from '@workflow/cbor-x';

const encoder = new Encoder({
  extensions: [
    {
      // Predicate function instead of Class constructor
      Class: (value) => value instanceof Headers || value?.constructor?.name === 'Headers',
      tag: 40004,
      encode(value, encode) {
        encode(Array.from(value));
      },
      decode(entries) {
...
### 3. Fixed VM context Error handling

The generic object tag (27) handler now properly uses `global.Error` as a fallback instead of the host's `Error` constructor, ensuring errors decoded in a VM context are instances of the VM's Error class.

---

# Original cbor-x README
